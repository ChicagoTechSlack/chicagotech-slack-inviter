...
export default function splash({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    !iframe && dom('footer',
      'Brought to you by ',
      dom('a href=http://www.spantree.net', 'Spantree, LLC'),
      dom('br'),
      dom('a class=spantree-logo href=http://www.spantree.net',
        dom(`img src=${path}assets/spantree-logo.png`)
      ),
      dom('br'),
      'Powered by ',
...
function style({ logo, active, large, iframe } = {}){
  ...
  css.add('.spantree-logo', {
    'border': '0'
  });

  css.add('.spantree-logo img', {
    'max-width': '130px',
    'margin': '1.2em'
  });
